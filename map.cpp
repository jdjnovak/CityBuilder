...
void Map::updateDirection(TileType tileType) {
  for (int y = 0; y < this->height; ++y) {
    for (int x = 0; x < this->width; ++x) {
      ...
      /* 
        BEWARE: LONG ASS PERIOD OF IF STATEMENTS
        TODO: REFACTOR THIS SECTION PL0X
      */
      // Check for same tile types on adj tiles if (x > 0 && y > 0)
	...
	adjacentTiles[2][7] = (this->tiles[(y + 1) * this->width + (x + 1)].tileType == tileType);
      ...
      // Change tile variant based on tile position
...
void Map::depthFirstSearch(std::vector<TileType>& whitelist, sf::Vector2i pos,
			   int label, int regionType = 0) {
...
void Map::findConnectedRegions(std::vector<TileType> whitelist, int regionType = 0) {
  ...
  for (int y = 0; y < this->height; ++y) {
	...
	depthFirstSearch(whitelist, sf::Vector2i(x, y), regions++);
